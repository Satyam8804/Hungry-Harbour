...
const RestaurantCard = ({ restaurantData,flex ,width}) => {
  ...
  return (
    <Link to={"/restaurants/" + id}>
      <div className={`relative sm:h-auto sm:w-[250px] w-${width} p-2 transition-transform duration-300 flex flex-${flex} sm:flex-col hover:scale-105 gap-4 justify-between`}>
        <img
          className="sm:w-full w-[150px] h-40 rounded-2xl transition-all ease-in-out duration-300 "
          src={IMG_URL + cloudinaryImageId}
          alt="res-logo"
        />
        <div className={`sm:full sm:px-4 flex flex-col`}>
